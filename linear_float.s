...
# Pós-Condição:
#   Avança 8 bytes em %edi (%edi = %edi + 8) e o empilha
# Registradores Alterados:
#   %edi
proximo_campo:
  popl return_addJ
...
# Pós-Condição:
#   Avança %ebx bytes em %edi (%edi = %edi + 8x%ebx) e o empilha
# Registradores Alterados:
...
alocar_matriz:
  ...
  mull %ecx # 8 x (%eax x %ebx), pois 1 float ocupa 8 bytes
...
determinante:
  ...
  ordem_maior_igual_3:
    ...
    loop_ordem_maior_igual_3:
      ...
      addl %eax, %edx # desloca 8*(indice_fcol-1) bytes para obter o elemento fixado
