...
\begin{center}
Relatório do Resolutor de Sistemas Lineares de\\N Variáveis em GNU Assembly
\end{center}
...
\end{titlepage}
	...
	O presente documento contém relatório do segundo trabalho da matéria ministrada pelo \prof
	...
	Tal seção busca apresentar a teoria na qual a implementação do Resolutor de Sistemas Lineares de $N$ variáveis se baseia. Não serão abordados aspectos referentes à linguagem de programação, tendo foco específico na técnica utilizada para resolver o problema em questão. Neste ínterim, utilizou-se o \textbf{Teorema de Cramer} para a resolução do sistema linear de $N$ variáveis e o \textbf{Teorema de Laplace} para os cálculos dos determinantes.
	...
	\label{sec:laplace}
	O Teorema (ou Regra) de Laplace consiste de uma técnica que define uma fórmula recursiva para o cálculo de determinantes de matrizes de ordem $n$ através do conhecimento imediato dos determinantes de suas \textbf{submatrizes}.

	\begin{figure}[h]
		\centering
		\includegraphics[width=0.6\linewidth]{laplace}
		\caption{Representação da aplicação da Regra de Laplace}
		\label{fig:laplace}
	\end{figure}
	...
	Assim, define-se a \textbf{Regra de Laplace}, através do cálculo recursivo dos determinantes. Nesse sentido, o caso base do Teorema de Laplace é quando $n = 1$ e $det(A) = |a_{11}| = a_{11}$.
	Contudo, há uma fórmula fechada\footnote{Entende-se por fechada a ausência da recursão.} para se calcular o determinante de matrizes de ordem $2$, também baseada no Teorema de Laplace.
	...
	\begin{itens}
		\item \textbf{Propósito}: Inserir o caractere de fim de string.
		\item \textbf{Pré-Condição}: O endereço da string está no topo da pilha.
		\item \textbf{Pós-Condição}: A string contém o caractere que sinaliza seu fim.
		\item \textbf{Registradores alterados}: -
	\end{itens}
	...
	\hline
	\&submatriz              \\ \hline
	det\_valor (det parcial) \\ \hline
	\%ecx (coluna fixa)      \\ \hline
	\%ebx (ordem da matriz)  \\ \hline
	\%edi (\&matriz)         \\ \hline
	\&retorno \hline
	\end{tabular}
	\label{tab:fr}
